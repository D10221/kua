...
import {AppContext, AppMiddleware, Auth} from './kontex';
...
describe('kompose auth + routing', () => {
    it('goes', (done) => {
        ...
        let a :Auth= new BasicAuth( new users.Service( new testing.UStore(crypto), crypto));

        app.use(router.get('/bye', bye));
        app.use(router.get('/hello', hello));
        app.use(a.lock(router.get('/admin', admin), ['admin']));
